...
export default function App() {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (hash.startsWith('#card=')) {
          ...
          return { ...DEFAULT_CARD_CONFIG, ...parsed };
        }
        ...
        if (saved) {
          ...
          return { ...DEFAULT_CARD_CONFIG, ...parsed, photos: cleanedPhotos };
        }
      } catch (e) {
  ...
  // Load config từ server khi có ?cid= trong URL
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    ...
    if (cid) {
      loadCardFromServer(cid).then(serverConfig => {
        if (serverConfig) {
          setConfig({ ...DEFAULT_CARD_CONFIG, ...serverConfig });
          setServerCardId(cid);
        }
      });
    }
  ...
  const handleUpdateVoiceNote = (updatedVoice: VoiceNoteData) => {
    setConfig(prev => ({
      ...prev,
